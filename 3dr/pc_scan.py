...
class AppLoop:
    ...
    def run(self):
        ...
        base_T_camera = self.base_T_camera
        file_count = []
        output_folder = './scan_output/scans'
        for filename in os.listdir(output_folder):
            if filename.endswith('.ply'):
                # find the file number
                base_name = filename[:-4]  # remove ".ply" 
                file_num = base_name[-3:]
                file_count.append(int(file_num))
        if not file_count:
            count = 1 
        else:
        ...
        while True:
            ...
            if key == ord("e"):
                ...
                append_line_to_txt('./scan_output/tcp.txt', str(base_T_hand))
                hand_T_base = np.linalg.inv(base_T_hand)
                hand_T_cam = hand_T_base @ base_T_camera
                append_line_to_txt('./scan_output/poses.txt', str(hand_T_cam)) #hand to camera pose
                depth_save_path = f'./scan_output/depth_image/depth_image{count}.png'
                color_save_path = f'./scan_output/color_image/color_image{count}.png'
