...
# base_T_camera = np.array([[-0.53723585,  0.04177973, -0.84239664,  0.85987956],
#  [ 0.84304113, -0.00380573, -0.53783563,  0.35412801],
#  [-0.02567657, -0.9991196,  -0.03317746,  0.28468093],
#  [ 0.,          0.,          0.,          1.        ]]) # Board 1
...
def gaussian_blur(image, kernel_size=(7, 7), sigma=1):
    """
    对图像进行高斯滤波
    :param image: 输入的单通道图像，类型为np.array
    :param kernel_size: 高斯核的大小，应为奇数，默认为(5, 5)
    :param sigma: 高斯核的标准差，默认为0，表示由kernel_size自动计算
    :return: 高斯滤波后的图像，类型为np.array
    """
...
while True:
    ...
    if key == ord("g"):
        log.info("Grasping gripper")
        gripper.move(0.11, 0.8, 60)
        time.sleep(3)
    ...
    if key == ord("e"):
        points.export_to_ply('./scan730_3/out_'+str(count).zfill(3)+'.ply', mapped_frame)
        robot.getRobotStates(robot_states)
        pose = robot_states.tcpPose
        base_T_hand = homogeneous_transform(pose[0:3], pose[3:7])
        append_line_to_txt('./scan730_3/tcp_poses.txt', str(base_T_hand))
        hand_T_base = np.linalg.inv(base_T_hand)
        hand_T_cam = hand_T_base @ base_T_camera
        append_line_to_txt('./scan730_3/h2c_poses.txt', str(hand_T_cam))
        depth_save_path = f'./scan730_3/depth_image{count}.png'
        color_save_path = f'./scan730_3/color_image{count}.png'
